...
#input data files
sales_data = read.csv("data/raw/sales_train.csv")
test_data = read.csv("data/raw/test.csv")
item_data = read.csv("data/raw/items.csv")
shops_data = read.csv("data/raw/shops.csv")
item_cat_data = read.csv("data/raw/item_categories.csv")
...
summary(mexp) #Multiple R-squared: 0.00031 Residual standard error 2.618
...
result3 = predict(gbm_model_1,newdata = test_data[4], n.trees = 2000 )

sub4= data.frame(ID = test_data$ID, 
                  item_cnt_month =  result3)
...
popular_shop = sales_data %>% group_by(shop_id) %>% summarise(Quantity_sold=sum(item_cnt_day)) %>%   ungroup() %>%
  arrange(desc(Quantity_sold)) %>% head(20) 
...
popular_cat = sales_data %>% group_by(item_category_id) %>%  summarise(Quantity_sold=sum(item_cnt_day)) %>%   ungroup() %>%
  arrange(desc(Quantity_sold)) %>% head(20) 
...
Most_Pop_comb = rbind(Mst_Pop_comb,Most_Pop_2013)  # combining most popular items for (2014,2015) with  2013
